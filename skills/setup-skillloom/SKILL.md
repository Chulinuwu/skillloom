...
On Windows, the bundled runner checks PATH and supported native, npm, and WinGet per-user locations for Claude Code and Codex. If setup reports the harness hosting the current conversation as `not-detected`, treat that as executable resolution failure rather than an absent harness. Refresh the current official installation and PATH guidance, resolve the CLI, and rerun the same setup command. Do not offer manual MCP configuration before bundled plugin installation has been exhausted with concrete evidence.
