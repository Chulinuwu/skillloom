...
test("compose exposes a writable Obsidian shell with managed Library and isolated Authoring", async () => {
  const compose = await read("hub/compose.yaml");
  ...
  assert.match(compose, /brain\/projections\/obsidian-vault\/Library:\/config\/Documents\/Skillloom\/Library:rw/u);
