...
async function validateManifests() {
  ...
  invariant(version === "0.3.10", "package version must be 0.3.10");
...
invariant(/native, npm, and WinGet per-user install locations[\s\S]*do not fall back to manual MCP configuration first/iu.test(agentInstructions), "AGENTS.md must prevent false Windows harness absence and premature manual MCP fallback");
...
async function validateSkills() {
  ...
  for (const name of skillNames) {
    ...
    invariant(/supported native, npm, and WinGet per-user locations[\s\S]*Do not offer manual MCP configuration/iu.test(source), `${name} must exhaust Windows harness resolution before manual MCP fallback`);
